...
namespace GenieLibrary
{
	...
	{
		/// <summary>
		/// The DAT hash this mapping is valid for. Length: Size of MD5-Hash, so always 16 bytes.
		/// </summary>
		...
		/// <summary>
		/// Creates a new empty mapping file.
		/// </summary>
		public MappingFile()
		{
			// Initialize members
			Hash = new byte[16];
			UnitMapping = new Dictionary<short, short>();
			ResearchMapping = new Dictionary<short, short>();
		}
		...
		/// <summary>
		/// Computes the hash of the given genie file.
		/// </summary>
		/// <param name="genieFile">The genie file to be hashed.</param>
		/// <returns></returns>
		public static byte[] ComputeHash(GenieFile genieFile)
		{
			...
			using(MemoryStream hashString = new MemoryStream())
			{
				...
				using(MD5 md5 = MD5.Create())
					return md5.ComputeHash(hashString);
			}
		}
		...
		/// <returns></returns>
		public bool CheckCompabilityToGenieFile(GenieFile genieFile)
		{
			// Calculate hash and compare
			return Hash.SequenceEqual(ComputeHash(genieFile));
		}
	}
}
